...
class DBSession(object):
    ...
    def __init__(self,host="operations.sdss.org",database="sdss5db",user="sdss_user"):
        """ Open the database connection."""
        connection = pg.connect(user=user,host=host,
                                password="",port = "5432",database = database)
    ...
    def query(self,table=None,cols='*',where=None,groupby=None,sql=None,fmt='numpy',verbose=False):
        ...
        # SQL command input
            ...
            cindex = create_index(colnames)
            bd,nbd = dln_where(cindex['num']>1)
            for i in range(nbd):
...
def dln_where(statement,comp=False):
